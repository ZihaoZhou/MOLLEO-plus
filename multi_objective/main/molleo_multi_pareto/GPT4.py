...
def query_LLM(messages, model="gpt-4.1-mini", temperature=0.0):
    ...
    for retry in range(3):
        try:
            obj = _get_client().chat.completions.create(**params)
...
class GPT4:
    ...
    def edit(self, mating_tuples, mutation_rate, min_evaluators, max_evaluators, boltz_cache, single_parent=False):
        ...
        #     return new_child
            ...
            objective=f"{task_objective} Only make a few modifications, then respond with FINAL_ANSWER. Do not let molecular weight exceed 700.\n"
            for i in range(10):
                try:
                    state = run_agent(parent_smiles[0], objective, max_steps=20)
                    break
                except Exception as e:
                    print(e)
                    continue
